...
if [ "`uname -s`" = "Darwin" ];then	#check macports locations for GNU versions, else no worky
	if [ -x /opt/local/bin/gshuf ];then
		shuf=/opt/local/bin/gshuf
	fi
	if [ -x /opt/local/bin/gsed ];then
		sed=/opt/local/bin/gsed
	fi
	if [ -x /opt/local/bin/wget ];then
		wget=/opt/local/bin/wget
	fi
	if [ -x /opt/local/bin/gawk ];then	
